...
_STRINGS: dict[str, dict[str, str]] = {
    "pt_BR": {
        ...
        "build.warn_no_state": "⚠️ Build falhou sem estado do latexmk — limpando cache para nova tentativa limpa.",
        "build.warn_state_kept": "⚠️ Build falhou mas estado do latexmk preservado — próxima compilação retomará incrementalmente.",
        # Onboarding
    ...
    "en_US": {
        ...
        "log.underfull": "⚠️ Loose spacing ({amount}) at lines {l1}–{l2}",
        "log.pkg_warn": "⚠️ [{pkg}] {msg}",
        "log.latex_warn": "⚠️ {msg}",
        "log.undefined_refs": "⚠️ Undefined references in document — recompile after fixing",
        "log.bib_style": "📝 Bibliography style: {sty}",
        "log.bib_db": "📚 Database: {db}",
        "log.no_citations": "ℹ️ No citations found in the document",
        "log.bib_errors": "✗ {msg}",
        "log.bib_empty_field": "⚠️ [BibTeX] Field '{field}' is empty in entry '{entry}'",
        ...
        "log.hint_undefined_cmd": "check for typo in command or missing \\usepackage",
        "log.hint_math_mode": "use $...$ around mathematical expressions",
        "log.hint_math_display": "check if the equation environment is properly closed",
        "log.hint_missing_open_brace": "add missing opening {",
        "log.hint_missing_close_brace": "add missing closing }",
        "log.hint_extra_brace": "unbalanced braces or $ — review the passage",
        "log.hint_too_many_braces": "there is a } with no matching {",
        "log.hint_runaway_arg": "unclosed argument — check your {}",
        ...
        "log.hint_usepackage_after_begin": "\\usepackage can only be used in preamble",
        ...
        "log.hint_env_undefined": "undefined environment — check required \\usepackage",
        "log.hint_too_many_floats": "add \\clearpage to flush accumulated floats",
        "log.hint_capacity_exceeded": "TeX internal limit exceeded — simplify the document",
        "log.hint_emergency_stop": "compilation aborted — see the previous error in the log",
        "log.hint_file_not_found": "file or package not found — check name or install with tlmgr",
        "log.hint_no_counter": "counter referenced before creation with \\newcounter",
        ...
        "log.hint_extra_right": "\\right with no matching \\left",
        "log.hint_missing_delimiter": "\\left with no valid delimiter — use \\left( or \\left.",
        "log.hint_display_math": "blank line inside equation — check \\[ \\]",
        "log.hint_dimension_large": "length value too large — check the unit",
        "log.hint_arithmetic_overflow": "overflow in calculation — simplify the expression",
        "log.hint_counter_large": "counter exceeds maximum — check lists or footnotes",
        "log.hint_missing_number": "missing number or length in argument",
        "log.hint_illegal_unit": "invalid unit of measure — use pt, cm, mm, em, ex, etc.",
        "log.hint_bad_math_delim": "\\[ without \\] or \\( without matching \\)",
        "log.hint_verb_newline": "\\verb cannot span a newline",
        "log.hint_caption_outside_float": "\\caption can only be used inside figure or table",
        "log.hint_missing_item": "\\item missing at start of list",
        "log.hint_lonely_item": "\\item outside a list environment",
        "log.hint_too_nested": "lists nested beyond 4 levels",
        "log.hint_not_outer_par": "float inside minipage, footnote or secondary environment",
        "log.hint_nfss": "NFSS font system misconfigured — check fontenc/fontspec",
        "log.hint_cant_find_file": "file not found — check path or filename",
        "log.hint_cant_write_file": "cannot write file — PDF may be open in another program",
        "log.hint_invalid_char": "invalid character in file — check encoding (use UTF-8)",
        "log.hint_illegal_param": "# misused — in URLs use \\#; in \\newcommand declare [N] params",
        "log.hint_wrong_driver": "PDF driver conflicts with hyperref + XeLaTeX",
