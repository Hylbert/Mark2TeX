...
%
%   times        → Times New Roman  (URW Nimbus Roman, métricas idênticas)
%   arial        → Liberation Sans  (métricas pixel-a-pixel idênticas à Arial)
%   helvetica    → Helvetica        (URW Nimbus Sans, via fonts-urw-base35)
%   ubuntu-sans  → Ubuntu Sans      (fonte da documentação Mark2TeX)
%
% Padrão (sem $font$): Times New Roman  — ABNT NBR 14724 recomendada
% -------------------------------------------------------------------
$if(font)$
$if(font)$\makeatletter\def\m@fontchoice{$font$}\makeatother$endif$
$endif$
